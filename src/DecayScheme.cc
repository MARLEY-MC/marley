...
const std::string marley::DecayScheme::ensdf_continuation_record = "[^ 1]";
const std::string marley::DecayScheme::ensdf_record_data = ".{71}";
const std::regex marley::DecayScheme::rx_ensdf_end_record("\\s*"); // Matches blank lines
const std::regex marley::DecayScheme::rx_paren_group("\\(([^)]+)\\)([+-])");
const std::regex marley::DecayScheme::rx_spin("([0-9]+(?:/2)?)");
const std::regex marley::DecayScheme::rx_jpi("([0-9]+(?:/2)?)([+-])?");
...
marley::Level* marley::DecayScheme::get_pointer_to_closest_level(double E_level) {
  ...
  marley::Level* plevel = pv_sorted_levels[e_index];
...
marley::DecayScheme::DecayScheme(std::string nucid, std::string filename,
  marley::DecayScheme::FileFormat ff)
{
...
// Load a level with marley::Gamma objects based on theoretical gamma-ray
// transition relative intensities
void marley::DecayScheme::assign_theoretical_RIs(marley::Level* level_i) {
...
void marley::DecayScheme::parse_ensdf(std::string filename) {
  ...
  while (!file_in.eof()) {
    ...
    if (std::regex_match(line, rx_primary_level_record)) {
      ...
      if (std::regex_search(jpi, match, rx_jpi)) {
        ...
        if (match.size() > 2) {
          if (match[2] == "+") parity = marley::Parity(1);
      ...
      p_current_level = add_level(marley::Level(level_energy,
        two_J, parity));
    }
  ...
  for(marley::Level* lev : pv_sorted_levels) {
    ...
    for(marley::Gamma& g : gammas) {
      ...
      size_t e_index = std::distance(sorted_level_energies.begin(),
      ...
      g.set_end_level(pv_sorted_levels[e_index]);
...
void marley::DecayScheme::parse_talys(std::string filename) {
  ...
  std::vector<double> level_energies; std::vector<marley::Level*> level_ps;
  ...
  for (int i = 0; i <= num_excited_levels; i++) {
    ...
    p_current_level = add_level(marley::Level(level_energy, twoJ, parity));
...
std::string marley::DecayScheme::process_continuation_records(std::ifstream &file_in,
...
void marley::DecayScheme::print_latex_table(std::ostream& ostr) {
  ...
  // Cycle through each of the levels owned by this decay scheme
  // object in order of increasing energy
  for(std::vector<marley::Level*>::iterator j = pv_sorted_levels.begin();
    j != pv_sorted_levels.end(); ++j)
  {
    ...
    // (according to the ENSDF specification, these will already be
    // sorted in order of increasing energy)
    for(std::vector<marley::Gamma>::iterator k = gammas.begin();
      k != gammas.end(); ++k)
    {
...
std::string marley::DecayScheme::get_nuc_id() const {
  return nuc_id;
}

void marley::DecayScheme::set_nuc_id(std::string id) {
  nuc_id = id;
}

// Adds a new level to the decay scheme and returns a pointer to it
marley::Level* marley::DecayScheme::add_level(marley::Level level) {
  ...
  marley::DecayScheme& ds)
{
  ...
  for (size_t i = 0; i < num_levels; ++i) {
    ...
    l = ds.add_level(marley::Level(energy, two_j, pi));
    for (size_t j = 0; j < num_gammas; ++j) {
      in >> energy >> ri >> level_f_idx;
      marley::Gamma& g = l->add_gamma(marley::Gamma(energy, ri, l));
      if (level_f_idx >= 0)
