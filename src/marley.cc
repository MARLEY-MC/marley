      ...
      private:
        ...
        virtual void open() override {
          ...
          if (mode_ == Mode::OVERWRITE) {
            ...
            tree_ = new TTree("MARLEY Event Tree",
            ...
            // is called.
            tree_->Branch("events", "marley::Event", nullptr);
          }

          else if (mode_ == Mode::RESUME) {
            file_->GetObject("MARLEY Event Tree", tree_);
            ...
            TNamed* seed = nullptr;
            file_->GetObject("MARLEY RNG seed", seed);
            ...
            if (!seed) MARLEY_LOG_WARNING() << "Unable to read"
              ...
              << " the random number generator seed " << seed->GetTitle();
          }
        ...
        {
          // Use TNamed objects to easily write strings to a ROOT file. See
          ...
          config.Write(nullptr, TNamed::kWriteDelete);
          state.Write(nullptr, TNamed::kWriteDelete);
          seed.Write(nullptr, TNamed::kWriteDelete);
        }
          ...
          long& num_previous_events) override
        {
          ...
          TNamed* conf = nullptr;
          file_->GetObject("MARLEY JSON configuration", conf);
          if (!conf) {
          ...
          marley::JSON config = marley::JSON::load(conf->GetTitle());

          TNamed* state = nullptr;
          file_->GetObject("MARLEY RNG state string", state);
          if (!state) {
            throw marley::Error("Cannot resume run. Failed to load"
          ...
          gen->seed_using_state_string(state->GetTitle());
        ...
        virtual void write_event(const marley::Event* event) override {
          ...
          tree_->SetBranchAddress("events", &event);
          tree_->Fill();
        }
