...
.. todo::
   ...
   This section was written for MARLEY v1.2.1 and describes the legacy
   native ASCII format, the HEPEVT format, the JSON format, and a ROOT
   format that uses the ``marley::Event`` class. Most of this content no
   longer accurately reflects the v2.0.0 codebase. The current supported
   output formats are "ascii" (HepMC3 text with NuHepMC v1.0.0
   conventions) and "root" (HepMC3 ``GenEventData`` in a ROOT TTree).
   The v2.0.0 update should replace this section entirely.
