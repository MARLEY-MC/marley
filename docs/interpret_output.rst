...
.. raw:: html
   ...
   </head><div class="highlight-none notranslate"><table class="highlighttable" id="ascii_format_example"><tr><td class="linenos"><div class="linenodiv"><pre> 1
    2
    3
    4
    5
...
gives the value of |xsecValueNatural| |InverseMeVSquaredPerAtom| =
...
A HEPEVT-format output files consists of one or more text-based event records.
...
``JDAHEP2`` entries record the indices 𝑗 (for 1 ≤ 𝑗 ≤ ``NHEP``) of particles in
the event record that correspond to the first mother, second mother, first
daughter, and last daughter of the current particle, respectively. These
indices are set to zero in cases where they do not apply (e.g., a particle
which has not decayed will have ``JDAHEP1`` = ``JDAHEP2`` = 0). Entries
``PHEP1`` through ``PHEP3`` record the x-, y-, and z-components of the particle
3-momentum, while ``PHEP4`` gives the total energy and ``PHEP5`` gives the
particle mass (all in GeV). Entries ``VHEP1`` through ``VHEP3`` store the x, y,
and z positions of the particle production vertex (mm), and ``VHEP4`` gives the
production time (mm/c).
...
section in units of |InverseMeVSquaredPerAtom| per atom. As is the case for the
...
.. literalinclude:: example.hepevt

.. |ascii_format_example| raw:: html

   <a href="#ascii_format_example">example file</a>

The listing above shows an example MARLEY output file in HEPEVT format. The
same two events from the ASCII-format |ascii_format_example| are used for easy
...
*key* is an arbitrary string delimited by double quotes (``""``), while each
*value* may be a string, a number, an object, an array, or one of the words
(without surrounding double quotes) ``true``, ``false``, or ``null``.
...
cross section (|InverseMeVSquaredPerAtom| per atom), and the integer random
