...
TMarleyNuclearReaction::TMarleyNuclearReaction(std::string filename,
...
void TMarleyNuclearReaction::set_decay_scheme(TMarleyDecayScheme* ds) {

  if (ds == nullptr) throw std::runtime_error(std::string("Null pointer")
    + " passed to TMarleyNuclearReaction::set_decay_scheme(TMarleyDecayScheme* ds)");
...
double TMarleyNuclearReaction::max_level_energy(double Ea) {
...
double TMarleyNuclearReaction::get_threshold_energy() {
  return Ea_threshold;
}

// Creates an event object by sampling the appropriate
// quantities and performing kinematic calculations
TMarleyEvent TMarleyNuclearReaction::create_event(int particle_id_a, double Ea,
  TMarleyGenerator& gen)
{
  ...
  TMarleyEvent event = make_event_object(Ea, pc_cm, cos_theta_c_cm, phi_c_cm,
    Ec_cm, Ed_cm, E_level);

  // Get a reference to the residue so that we can handle its de-excitation
  TMarleyParticle& residue = *event.get_residue();
...
double TMarleyNuclearReaction::total_xs(int particle_id_a, double Ea) {
...
double TMarleyNuclearReaction::total_xs(double E_level, double Ea,
...
double TMarleyNuclearReaction::sample_cos_theta_c_cm(/*double matrix_el,*/
  int m_type, double beta_c_cm, TMarleyGenerator& gen)
{
