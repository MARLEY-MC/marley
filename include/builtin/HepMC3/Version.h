...
#define HEPMC3_DECLARE_READER_SPSTREAM(classname)  extern "C" {  __declspec(dllexport) classname * __stdcall new ## classname ## spstream (std::shared_ptr<std::istream> stream) { return new classname (stream);  } }
...
#define HEPMC3_DECLARE_WRITER_SPSTREAM(classname)  extern "C" { classname * new ## classname ## spstream (std::shared_ptr<std::ostream> stream, std::shared_ptr<GenRunInfo> run) { return new classname (stream, run);  } }
#endif
#endif
