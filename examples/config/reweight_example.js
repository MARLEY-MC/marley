...
// JSON-like syntax as the other MARLEY commands. Only the "reweight"
// top-level section (with "weights" and "output" subsections) is read from
// the configuration file. All other parameters needed by the Generator are
// automatically restored from the "MARLEY.JSONconfig" attribute saved in the
// input event file.
//
{ // An opening curly brace begins the configuration file content

  // REWEIGHT SECTION (required)
  //
  // All configuration for the "marley reweight" command is contained within
  // the "reweight" top-level JSON object. It has two mandatory keys:
  // "weights" (an array of weight calculator configurations) and "output"
  // (an array of output stream configurations).
  reweight: {
